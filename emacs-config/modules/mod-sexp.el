  ...
  (let ((scratch-buffer-name  "*scratch*")
    ...
    (if (equal (buffer-name (current-buffer)) scratch-buffer-name)
      ...
      (with-current-buffer (switch-to-buffer  scratch-buffer-name)
        ...
        (goto-char (point-max))))))
;;
(defun sexp-mode-init()
  (interactive)
  (message "sexp-mode-init with:%s" (symbol-name major-mode))
  ;; `rainbow'
...
(add-hook 'eval-expression-minibuffer-setup-hook #'sexp-mode-init)
(add-hook 'emacs-mode-hook            #'sexp-mode-init)
(add-hook 'emacs-lisp-mode-hook       #'sexp-mode-init)
(add-hook 'inferior-emacs-mode-hook   #'sexp-mode-init)
(add-hook 'ielm-mode-hook             #'sexp-mode-init)
(add-hook 'lisp-mode-hook             #'sexp-mode-init)
(add-hook 'lisp-interaction-mode-hook #'sexp-mode-init)
(add-hook 'common-lisp-mode-hook      #'sexp-mode-init)
(add-hook 'scheme-mode-hook           #'sexp-mode-init)
(add-hook 'hy-mode-hook               #'sexp-mode-init)
(add-hook 'newlisp-mode-hook          #'sexp-mode-init)
(add-hook 'hy-mode-hook               #'sexp-mode-init)
(add-hook 'carp-mode-hook             #'sexp-mode-init)
(add-hook 'slime-mode-hook            #'sexp-mode-init)
(add-hook 'cider-mode-hook            #'sexp-mode-init)
(add-hook 'cider-repl-mode-hook       #'sexp-mode-init)
(add-hook 'clojure-mode-hook          #'sexp-mode-init)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(provide 'mod-sexp)
